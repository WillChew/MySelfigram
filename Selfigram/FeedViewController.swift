        ...
        if let image = info[UIImagePickerControllerOriginalImage] as? UIImage {
            ...
            if let imageData = UIImageJPEGRepresentation(image, 0.9),
                ...
                post.saveInBackground(block: {(success, error) -> Void in
                    if success {
                        print("Post Successfuly saved in Parse")
                        
                        //3. Add post to our posts array, choose index 0 so that it will be added
                        //to the top of the table instead of at the bottom (default behaviour)
                        self.posts.insert(post, at:0)
                        
                        //4. Now that we added a post, updating our table
                        //we are just inserting our new Post instead of reloading our whole tableView
                        //both methods would work, however, this gives us a cool animation for free
